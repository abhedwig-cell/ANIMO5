...
! Authority: ANIMO-TIME02@b4d78cf32cb149cf50aa2b0a0fbefee571eee6b8.
! Newly written bounded exact-integer realization of TIME02 candidate semantics.
! No SWAP5 time representation is ported.
module mod_animo_time_coordinate
  ...
  public :: serialize_time_coordinate
  public :: deserialize_time_coordinate
...
contains
  ...
  subroutine time_compare(left, right, ordering, ok)
    ...
    ordering = 0
    ok = .false.
    if (.not. time_is_valid(left)) return
    if (.not. time_is_valid(right)) return
    ...
    ordering = compare_nonnegative_ratios( &
      left%subday_numerator, left%subday_denominator, &
  ...
  subroutine time_add_fraction(base, add_numerator, add_denominator, result, ok)
    ...
    if (add_denominator <= 0_int64) return
    if (add_numerator < 0_int64) return
  ...
  subroutine serialize_time_coordinate(value, text, ok)
    type(TimeCoordinate), intent(in) :: value
    ...
    text = ''
    ok = .false.
    if (.not. time_is_valid(value)) return
    write(buffer, '(A,"|",I0,"|",I0,"|",I0)', iostat=ios) &
      trim(value%calendar_contract_id), value%day_index, &
      value%subday_numerator, value%subday_denominator
    if (ios /= 0) return
    if (len_trim(buffer) > len(text)) return
    text = trim(buffer)
    ok = .true.
  end subroutine serialize_time_coordinate

  subroutine deserialize_time_coordinate(text, value, ok)
    character(len=*), intent(in) :: text
    type(TimeCoordinate), intent(out) :: value
    logical, intent(out) :: ok
    integer :: p1, p2, p3, ios
    character(len=CALENDAR_ID_LEN) :: calendar
    integer(int64) :: day_index, numerator, denominator
    ...
    if (ios /= 0) return
    read(text(p3+1:), *, iostat=ios) denominator
    if (ios /= 0) return

    call make_time_coordinate(trim(calendar), day_index, numerator, denominator, value, ok)
  end subroutine deserialize_time_coordinate
...
end module mod_animo_time_coordinate
