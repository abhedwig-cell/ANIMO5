subroutine ghg_phase_kernel(Itask,Ioptmp,LnBoMp,LnTpMpSr,Nl,St,Flib,Flio,Flou,He,Mofr,Mofro,Mofrsa,Mofrt,Mofrox,MofrsaMp,SrWaMpCp,Mofrtx,Floux,Mofrx,Mofrsax,Flaiib,Flaiio,Flaiou,FlaiAtmos,CapFloux,CapMofrx,CapMofrsax,CapFlaiib,CapFlaiio,CapFlaiou,CapFlaiAtmos)
  implicit none
  ...
  real :: FlaiAtmos,Flaiib(0:Manl),Flaiio(0:Manl),Flaiou(0:Manl),Flair(Manl+1),Floux(0:Manl),Mofrx(0:Manl),Mofrsax(0:Manl)
...
1000 continue
  ...
  do while (La.gt.0.and.La.lt.Nl.and.Mofrsax(La)-Mofrx(La).gt.1.e-7)
    La=La+1
  enddo
  La=max(0,La-1); Flair(La+1)=0.0
  do Ln=La,1,-1
    Flair(Ln)=Flair(Ln+1)+(Mofrox(Ln)-Mofrtx(Ln))*He(Ln)*St
  enddo
  do Ln=La+2,Nl; Flair(Ln)=0.0; enddo
  do Ln=1,Nl
...
subroutine GHGMapohydro_k(LnBoMp,LnTpMpSr,Nl,Flib,Flio,Flou,He,Mofr,Mofrox,MofrsaMp,Mofrt,SrWaMpCp,Floux,Mofrx,Mofrsax,Mofrtx)
  ...
  do Ln=LnTp,max(LnBoMp(1),LnBoMp(2))
    ...
    Mofrx(Ln)=0.5*(Mofrox(Ln)+Mofrtx(Ln)); Rest=Flib(Ln)+Flio(Ln)-(Mofrtx(Ln)-Mofrox(Ln))*He(Ln); Floux(Ln)=Flou(Ln)+max(0.0,Rest)
  enddo
end subroutine
