...
ANIMO, qualify B2 behaviour, admit B3/B4, qualify canonical STATE/TIME/MASS/EX,
...
EXPECTED = {
    ...
    "NQ02": ("work/animo-nq02-tcd019-nonlinear-p-qualification", "12e874559d484417cea1ca5d4ef719ea0b359585"),
    ...
    "TIMEQ02": ("work/animo-timeq02-adapter-transaction-fixtures", "6b19e55f93fd84f1f956fd8e5a32034235b39588"),
...
REQUIRED_GATES = {
    "THEORY", "TESTING", "NUMERICAL", "TEMPORAL", "EXCHANGE_RUNTIME",
    "ARCHITECTURE", "TCD016_SCIENCE", "MACROPORE", "GHG", "B3_GOVERNANCE",
...
def main() -> int:
    ...
    try:
        ...
        timeq02_attachment_path = root / "integration/animo-reg/g5/ANIMO_RG02_G5_TIMEQ02_ATTACHMENT.json"
        required_paths = [
            register_path,
            gate_path,
            doc_path,
        ...
        require(by_id["TIMEQ02"]["canonical_gate_status"] == "CANONICAL_EXCHANGE_NOT_ADMITTED", "TIMEQ02 must not admit canonical EX")
        require(by_id["TIMEQ01"]["b2_status"] == "NOT_QUALIFIED" and by_id["TIMEQ02"]["b2_status"] == "NOT_QUALIFIED", "synthetic runtime evidence must not become B2")
        require(by_id["TIMEQ01"]["b3_status"] == "NOT_ADMITTED" and by_id["TIMEQ02"]["b3_status"] == "NOT_ADMITTED", "synthetic runtime evidence must not become B3")
        ...
        require("ARCHG02 independently resolves" in archg_note and "G8 remains unadmitted" in archg_note, "ARCHG02 temporal revalidation resolution not explicit")
        ...
        for fragment in ["38/38 synthetic", "no real SWAP/WOFOST producer", "H016 T008 T009 deferred", "no B2 canonical EX"]:
            require(fragment in by_id["TIMEQ02"]["notes"], f"TIMEQ02 evidence guard missing: {fragment}")
        ...
        require("TIMEQ01" in gates["TEMPORAL"]["input_streams"] and "TIMEQ02" in gates["TEMPORAL"]["input_streams"], "TIMEQ01/TIMEQ02 missing from TEMPORAL gate")
        require(gates["EXCHANGE_RUNTIME"]["input_streams"] == "TIMEQ02" and gates["EXCHANGE_RUNTIME"]["g5_state"] == "ATTACHED_BLOCKED", "TIMEQ02 exchange-runtime gate incorrect")
        require("ARCHG02" in gates["ARCHITECTURE"]["input_streams"] and "TIMEQ01" in gates["ARCHITECTURE"]["input_streams"] and "TIMEQ02" in gates["ARCHITECTURE"]["input_streams"], "ARCHG02/TIMEQ01/TIMEQ02 missing from architecture gate")
        require("ARCHG02" in gates["G8_ARCHITECTURE"]["input_streams"] and "TIMEQ01" in gates["G8_ARCHITECTURE"]["input_streams"] and "TIMEQ02" in gates["G8_ARCHITECTURE"]["input_streams"], "ARCHG02/TIMEQ01/TIMEQ02 missing from G8")
        require("TIMEQ02" in gates["G6_B2"]["input_streams"], "TIMEQ02 B2 non-substitution not represented in G6")
        ...
        for fragment in [
            ...
            "TIMEQ02@6b19e55f93fd84f1f956fd8e5a32034235b39588",
            "38 are executed by TIMEQ02 and all 38 pass",
            "H016",
            "T008",
            "T009",
            "does not admit canonical EX",
            ...
            "hidden cross-invocation task-state dependencies",
        ]:
        ...
        timeq1 = json.loads(timeq01_attachment_path.read_text(encoding="utf-8"))
        require(timeq1.get("head") == EXPECTED["TIMEQ01"][1], "TIMEQ01 attachment head mismatch")
        require(timeq1.get("decision") == "QUALIFIED_CANDIDATE_RUNTIME_TRANSACTION_AND_SCHEDULER_TRACE_FIXTURE_NON_B2_NONPRODUCTION", "TIMEQ01 decision mismatch")
        require(timeq1.get("verification", {}).get("conclusion") == "success", "TIMEQ01 CI not successful")
        require(timeq1.get("verification", {}).get("tests_passed") == 19 and timeq1.get("verification", {}).get("tests_failed") == 0, "TIMEQ01 test count mismatch")
        require(timeq1.get("evidence", {}).get("class") == "ARCH_RUNTIME_SYNTHETIC", "TIMEQ01 evidence class mismatch")
        require(timeq1.get("evidence", {}).get("b2_reference") is False, "TIMEQ01 attachment must preserve B2=false")
        effect1 = timeq1.get("g5_effect", {})
        for key in ["canonical_state_admitted", "canonical_time_admitted", "b3_scientific_admission", "b4_baseline_admitted", "production_migration_admitted"]:
            require(effect1.get(key) is False, f"TIMEQ01 attachment improper admission: {key}")
        require(effect1.get("actual_animo_process_kernel_connected") is False and effect1.get("real_external_owner_adapter_executed") is False, "TIMEQ01 must not claim real kernel/adapter execution")
        checks.append({"check": "timeq01_attachment_boundaries", "result": "PASS"})

        timeq2 = json.loads(timeq02_attachment_path.read_text(encoding="utf-8"))
        require(timeq2.get("head") == EXPECTED["TIMEQ02"][1], "TIMEQ02 attachment head mismatch")
        require(timeq2.get("decision") == "QUALIFIED_CANDIDATE_EXTERNAL_OWNER_ADAPTER_RUNTIME_FIXTURE_SYNTHETIC_NON_B2_NONPRODUCTION", "TIMEQ02 decision mismatch")
        require(timeq2.get("verification", {}).get("conclusion") == "success", "TIMEQ02 CI not successful")
        require(timeq2.get("verification", {}).get("cases_executed") == 38 and timeq2.get("verification", {}).get("cases_deferred") == 3, "TIMEQ02 case disposition mismatch")
        require(timeq2.get("verification", {}).get("tests_passed") == 38 and timeq2.get("verification", {}).get("tests_failed") == 0, "TIMEQ02 test count mismatch")
        require(timeq2.get("evidence", {}).get("class") == "ADAPTER_RUNTIME_SYNTHETIC_PRODUCER", "TIMEQ02 evidence class mismatch")
        require(timeq2.get("evidence", {}).get("b2_reference") is False, "TIMEQ02 attachment must preserve B2=false")
        effect2 = timeq2.get("g5_effect", {})
        for key in ["canonical_state_admitted", "canonical_time_admitted", "canonical_mass_admitted", "canonical_exchange_admitted", "b3_scientific_admission", "b4_baseline_admitted", "production_migration_admitted"]:
            require(effect2.get(key) is False, f"TIMEQ02 attachment improper admission: {key}")
        require(timeq2.get("evidence", {}).get("actual_animo_process_kernel_connected") is False, "TIMEQ02 must not claim ANIMO process-kernel execution")
        require(timeq2.get("evidence", {}).get("real_swap_adapter_executed") is False and timeq2.get("evidence", {}).get("real_wofost_or_other_crop_adapter_executed") is False, "TIMEQ02 must not claim real producer adapter execution")
        require(set(timeq2.get("deferred_arch07_cases", {})) == {"H016", "T008", "T009"}, "TIMEQ02 deferred case set mismatch")
        checks.append({"check": "timeq02_attachment_boundaries", "result": "PASS"})
