...
B3I09 = "44f29974a1f49cffd36f67857a1ed2c919978a07"
...
OLD_REVIEWED_HEAD = "32b549c68e89a44fc7bc99cfc209d905b7010d46"
FAILED_CLOSEOUT = "8a6699788d23179c08dd9417016731118b69aaf4"
...
ALLOWED = {
    ...
    "integration/animo-b3/ANIMO-B3COMP01_INTERNAL_ADVERSARIAL_REREVIEW.json",
...
assert_eq(pins["prior_tcd025_routing"]["head"], B3I08, "B3I08 pin")
assert_eq(pins["current_qualified_tcd025_parent_scope_routing"]["head"], B3I09, "B3I09 qualified pin")
assert_eq(pins["current_qualified_tcd025_parent_scope_routing"]["exact_final_ci_run"], 34585740977, "B3I09 exact-final CI pin")
...
assert_eq(pins["superseded_b3comp01_checkpoint"]["reviewed_head"], OLD_REVIEWED_HEAD, "old reviewed head")
assert_eq(pins["superseded_b3comp01_checkpoint"]["failed_closeout_head"], FAILED_CLOSEOUT, "failed closeout pin")
...
assert_eq(heads.get(b3i09_ref), B3I09, "B3I09 live head advanced after re-audit; fresh audit required")
...
assert_eq(b3i09["state"], "QUALIFIED_A5_EXCLUDED_RESTRICTED_PARENT_READY_FOR_SEPARATE_COMPOSITION_NO_ADMISSION", "B3I09 qualified state")
assert_eq(b3i09["scientific_admission"], False, "B3I09 no scientific admission")
assert_eq(b3i09["parent_tcd_admitted"], False, "B3I09 parent non-admission")
assert_eq(b3i09["A5_b3_admitted"], False, "B3I09 A5 non-admission")
assert_eq(b3i09_route["A5_disposition"]["state"], "PERMANENTLY_EXCLUDED_FROM_SUPPORTED_PARENT_SCOPE_FOR_FROZEN_REVISION53", "B3I09 A5 exclusion")
assert_eq(b3i09_route["A5_disposition"]["scientifically_source_closed"], False, "B3I09 no A5 source closure")
assert_eq(b3i09_route["parent"]["admitted"], False, "B3I09 parent admission")
assert_eq(b3i09_route["parent"]["ready_for_separate_restricted_parent_composition_decision"], True, "B3I09 separate parent decision")
...
assert_eq(audit["observations"]["b3i09_exact_final_green"], True, "audit B3I09 exact-final observation")
assert_eq(audit["observations"]["tcd025_parent_admitted"], False, "audit parent observation")
...
old_review_path = ROOT / "integration/animo-b3/ANIMO-B3COMP01_INTERNAL_ADVERSARIAL_REVIEW.json"
new_review_path = ROOT / "integration/animo-b3/ANIMO-B3COMP01_INTERNAL_ADVERSARIAL_REREVIEW.json"
if not old_review_path.exists():
    raise AssertionError("superseded first review artifact must be retained as failure-history evidence")
old_review = load("integration/animo-b3/ANIMO-B3COMP01_INTERNAL_ADVERSARIAL_REVIEW.json")
assert_eq(old_review["reviewed_head"], OLD_REVIEWED_HEAD, "superseded review identity")

if status["phase"] == "AUTHORING_FROZEN_PENDING_GOV05_ADVERSARIAL_REREVIEW":
    ...
    if new_review_path.exists():
        raise AssertionError("fresh rereview artifact present before remediated authoring checkpoint")
    print("PASS_B3COMP01_REMEDIATED_AUTHORING_FROZEN_NEGATIVE_COMPLETENESS_AUDIT")
    ...
    assert_eq(status["review"]["completed"], True, "final rereview completion")
    if not new_review_path.exists():
        raise AssertionError("fresh rereview artifact missing")
    review = load("integration/animo-b3/ANIMO-B3COMP01_INTERNAL_ADVERSARIAL_REREVIEW.json")
    assert_eq(review["outcome"], "SELF_REREVIEW_PASS_NEGATIVE_COMPLETENESS_AUTHORITY", "rereview outcome")
    assert_eq(review["assurance"], "PROCESS_SELF_REVIEWED_NOT_INDEPENDENT", "rereview assurance")
    assert_eq(review["genuinely_independent"], False, "rereview independence")
    authoring_head = status["validation"]["remediated_authoring_head"]
    assert_eq(review["reviewed_head"], authoring_head, "rereviewed authoring head")
    ...
    if post_review_changed - permitted:
        raise AssertionError(f"substantive files changed after fresh reviewed checkpoint: {sorted(post_review_changed - permitted)}")
    ...
    print("PASS_B3COMP01_NEGATIVE_COMPOSITION_COMPLETENESS_AUTHORITY_AFTER_CONCURRENCY_REAUDIT")
else:
