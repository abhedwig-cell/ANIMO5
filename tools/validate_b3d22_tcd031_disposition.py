...
# Identity and exact authoring base.
...
req(d["current_aggregate_at_open"] == f"ANIMO-RG05H@{RG05H}", "aggregate-at-open drift")
req(d["current_review_governance"] == f"ANIMO-GOV05@{GOV05}", "current GOV05 authority drift")
...
req(d["gov04_risk_tier_retained_by_gov05"] == "C", "Tier-C classification drift")
req(d["admission_route"] == "INDEPENDENT_SCIENTIFIC_ADMISSION_WITH_HISTORICAL_UNCERTAINTY", "B3Q01 route drift")
req(d["admitted"] is False, "disposition performed admission")

# Exact authority pins.
a = d["authorities"]
req(a["aggregate"] == f"ANIMO-RG05H@{RG05H}", "aggregate pin drift")
req(a["GOV05"] == f"ANIMO-GOV05@{GOV05}", "GOV05 pin drift")
req(a["GOV05_exact_head_CI"] == f"{GOV05_CI}:success", "GOV05 exact-head CI pin drift")
req(a["GOV04"] == f"ANIMO-GOV04@{GOV04}", "GOV04 pin drift")
req(a["GOV03"] == f"ANIMO-GOV03@{GOV03}", "GOV03 pin drift")
req(a["B3Q01"] == f"ANIMO-B3Q01@{B3Q01}", "B3Q01 pin drift")
req(a["routing"] == f"ANIMO-B3I07@{B3I07}", "routing pin drift")
...
req(g5s["work_unit"] == "ANIMO-GOV05", "wrong GOV05 authority object")
req(g5s["decision"] == "QUALIFIED_SINGLE_AGENT_ADVERSARIAL_REVIEW_GOVERNANCE_WITH_EXPLICITLY_REDUCED_INDEPENDENCE_ASSURANCE_NO_SCIENTIFIC_GATE_REDUCTION", "GOV05 decision drift")
req(g5s["work_status"]["qualified"] is True and g5s["work_status"]["work_unit_complete"] is True, "GOV05 not qualified/complete")
req(g5m["prospective_only"] is True, "GOV05 no longer prospective-only")
...
req("IF_IT_COMPLETES_KEEP_ITS_ORIGINAL_STATUS" in items[0]["transition"], "GOV05 transition does not preserve completed R2 status")
gt = d["governance_transition"]
for key in ("gov05_prospective_only", "gov05_qualified_before_b3d22_authoring", "b3b10r2_completed_before_gov05_qualification", "b3b10r2_original_gov04_status_retained", "completed_gov04_independent_review_remains_valid_under_gov05"):
    req(gt[key] is True, f"governance transition predicate false: {key}")
req(gt["b3b10r2_relabelled_as_same_agent_review"] is False, "R2 relabelled as same-agent review")
req(gt["scientific_gate_reduction"] is False, "scientific gate reduction recorded")

# R2 is a completed PASS, while first B3B10R remains historical fail-closed authority.
r2 = git_show_json(R2, "integration/animo-b3/ANIMO-B3B10R2_STATUS.json")
req(r2["workunit"] == "ANIMO-B3B10R2" and r2["tcd"] == "TCD-031", "wrong R2 identity")
req(r2["review_disposition"] == EXPECTED_REVIEW, "R2 targeted rereview did not PASS")
req(r2["independent_tier_c_review_gate"] == "PASSED", "R2 Tier-C review gate not passed")
...
req(r1["review_status"] == "COMPLETE_FAIL_CLOSED" and r1["review_disposition"] == "REMEDIATION_REQUIRED", "original B3B10R history drift")
req(r1["scientific_falsification"] is False, "original B3B10R scientific falsification drift")

# Readiness and STATEQ04 define the same atomic state model.
...
req(ready["gov04_risk_tier"] == "C", "B3B10 risk tier drift")
req(ready["atomic_scope"]["identity"] == ATOMIC_ID, "B3B10 atomic identity drift")
req(ready["atomic_scope"]["p_off_scalars"] == 8 and ready["atomic_scope"]["p_on_scalars"] == 12, "B3B10 state-count drift")
req(ready["atomic_scope"]["writer_and_restore_halves_separate_admissions"] is False, "B3B10 permits split correction")
q4 = git_show_json(STATEQ04, "integration/animo-state/STATEQ04_REMEDIATION_CONTRACT.json")
req(q4["checkpoint_boundary"] == "ACCEPTED_BOUNDARY_ONLY" and q4["domain_count"] == 2, "STATEQ04 boundary/domain drift")
req(q4["persistent_coordinates"]["p_off_scalar_count"] == 8 and q4["persistent_coordinates"]["p_on_scalar_count"] == 12, "STATEQ04 state-count drift")
...
# Formal scientific object remains exactly atomic.
sc = d["atomic_scientific_disposition"]
req(sc["identity"] == ATOMIC_ID, "formal atomic identity drift")
req(sc["domain_count"] == 2 and sc["p_off_scalar_count"] == 8 and sc["p_on_scalar_count"] == 12, "formal state-count drift")
req(sc["phosphorus_condition"] == "IPO.EQ.1" and sc["result_alias"] == "RsCoMp*", "formal P/result-alias drift")
req(sc["serializer_only_is_sufficient"] is False and sc["restore_only_is_sufficient"] is False, "half-correction incorrectly accepted")
...
# Claim-scoped split evidence supports disposition, not production-bound equivalence.
...
req(ce["comparison_policy"] == "EXACT_BYTEWISE_NO_TOLERANCE" and ce["fresh_stage_b_process"] is True, "comparison/fresh-process drift")
req(ce["whole_model_active_production_split_equivalence"] == "NOT_PROVEN", "whole-model split overclaimed")
req(ce["whole_model_production_split_required_for_this_formal_disposition"] is False, "disposition evidence boundary drift")
...
# GOV03 historical uncertainty remains fail-closed.
...
req(g3["hard_boundaries"]["historical_B2_recovered"] is False, "GOV03 reports B2")
h = d["historical_uncertainty"]
req(h["qualified_b2_available"] is False and h["historical_revision53_active_macropore_restart_behaviour"] == "UNKNOWN_WITHOUT_B2", "historical behavior overclaimed")
...
# GOV05 preserves GOV04 Tier-C scientific risk gates while replacing future review-process assurance.
...
req(g5m["gov04_gate_preservation"]["C"]["review_process_gate_replacement"]["scientific_evidence_gate_removed"] is False, "GOV05 removed a Tier-C scientific gate")
req(g5m["tiers"]["C"]["review_mode"] == "ENHANCED_INTERNAL_ADVERSARIAL_REVIEW", "future Tier-C GOV05 review mode drift")
tier = d["tier_c_disposition"]
req(tier["gov05_review_process_replacement_applied_to_completed_r2"] is False, "GOV05 future review mode applied retroactively to R2")
req(tier["gov05_completed_review_transition_applied"] == "KEEP_ORIGINAL_GOV04_REVIEW_STATUS_AND_ACCEPT_AS_VALID_COMPLETED_REVIEW", "completed-review transition drift")
req(tier["separate_admission_decision_required_by_this_workunit_contract"] is True, "disposition improperly combines admission")
req(tier["tier_d_trigger_found"] is False and tier["composition"] is False and tier["production_bound_scope"] is False, "Tier-D/composition/production scope leaked in")
req(tier["numerical_policy_change"] is False and tier["solver_or_tolerance_change"] is False, "numerical policy leaked in")

# Canonical TCD-031 remains OPEN at pinned routing authority.
...
req(rows31[0].get("process") == "macropore persistent solute restart state", "canonical process identity drift")
req(rows31[0].get("classification") == "RESERVED_POST_G5_MISSING_RESTART_STATE_REPRESENTATION", "canonical classification drift")

# Expected-difference and hard boundaries cannot widen.
e = d["expected_difference_surface"]
req(e["whole_model_equivalence_claimed"] is False and e["production_restart_equivalence_claimed"] is False, "whole-model/production equivalence overclaimed")
req(e["global_arbitrary_tolerance_allowed"] is False, "arbitrary tolerance admitted")
...
for key, value in d["hard_boundaries"].items():
    req(value is False, f"forbidden disposition side effect: {key}")

# Status may claim final qualification only after its recorded validation state is green.
...
req(s["current_review_governance"] == f"ANIMO-GOV05@{GOV05}", "status current governance drift")
req(s["independent_rereview_result"] == EXPECTED_REVIEW and s["atomic_correction_identity"] == ATOMIC_ID, "status review/atomic identity drift")
...
sg = s["governance_transition"]
req(sg["gov05_qualified_before_b3d22_authoring"] is True and sg["b3b10r2_completed_before_gov05_qualification"] is True, "status GOV05 timing drift")
req(sg["b3b10r2_original_gov04_status_retained"] is True and sg["b3b10r2_relabelled_as_same_agent_review"] is False, "status R2 transition drift")
for key, value in s["hard_boundaries"].items():
    req(value is False, f"forbidden status side effect: {key}")
if s["validation"]["machine_validated"]:
    req(s["qualified"] is True, "validated status not qualified")
...
print("B3D22 PASS: TCD-031 Tier-C disposition is GOV05-current, completed-GOV04-review preserving, atomic, historically uncertainty-bounded, production-bounded, and not admitted.")
