...
| FPE-003 | `TRANSPORT.FOR:264`, relative balance deviation divided by `max(abs(Bapd),abs(Batr))` | static source risk; no BUILDQ01 invalid observation established | `HISTORICAL_EFFECT_UNKNOWN`; potential `IEEE_FLAG_ONLY` or `CONTROL_FLOW_MATERIAL` if zero scale is reachable | `HISTORICAL_EFFECT_UNKNOWN` | Targeted reachability probe required before defect admission. |
| FPE-004 | `Transgen.for:379`, same relative balance pattern | static source risk only | `HISTORICAL_EFFECT_UNKNOWN` | `HISTORICAL_EFFECT_UNKNOWN` | Same as FPE-003. |
| FPE-005 | `ghgtransport.for:263`, gas-transport balance relative deviation | static source risk only; authoritative natural GHG runtime remains reference-limited | `HISTORICAL_EFFECT_UNKNOWN`; potential diagnostic/control-flow materiality | `HISTORICAL_EFFECT_UNKNOWN` | Probe only after a qualified GHG runtime case is available. |
