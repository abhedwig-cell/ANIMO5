...
The comparator itself performs no normalization. For each observation it aligns B2 and B1 by:
...
A successful representation comparison does not establish numerical equivalence. It only shows that the recorded representation observations match exactly.

## Relationship to the other NQ01 tools

The first B2 exercise should use three layers:
...
The first-reference packet validator then checks that the evidence chain is complete without converting any comparator result into automatic admission.
