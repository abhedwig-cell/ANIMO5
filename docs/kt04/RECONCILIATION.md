...
The project declares `LocalVariableStorage="localStorageSave"` in all four configurations and `LocalSavedScalarsZero="true"` specifically for `Debug|x64`. The supplied executable embeds the PDB path `D:\USR\5200048928_SWAP_ANIMO\ANIMO\x64\Debug\animo41.pdb`, is PE32+ x86-64, and carries a 2026 linker timestamp. This is consistent with zero-valued unread saved scalars in that supplied debug build.

This evidence is supplementary only. The executable is a modern rebuild, not a recovered revision-53 historical reference. KT04 therefore does not use it to replace the KT03F01 scientific/source disposition or to claim B2 historical behaviour. It merely shows that the qualified absent-state arithmetic contribution is compatible with the supplied debug-build configuration.
