...
## External function result audit

The systematic source-bound external-function result audit requested in the previous revision is now implemented in `tools/audit_fortran_function_interfaces.py` and persisted in `docs/prep01/EXTERNAL_FUNCTION_INTERFACE_AUDIT.csv`.

The auditor verifies the frozen source-archive SHA-256 before analysis and is deliberately classified as a static source-text audit rather than a complete Fortran semantic parser.

Current result over all 65 source/include files:

- 19 external function definitions have an explicitly reconstructable result type;
- 18 have observed explicit declarations consistent with the definition;
- only `Dble_trunc` has mixed explicit declarations;
- `Dble_trunc` is defined as `REAL(8)` and is observed as both `REAL` and `REAL(8)` in explicit declarations;
- 7 explicit `Dble_trunc` declarations and 105 conservative possible call occurrences are observed;
- under the evidence-derived eight-byte-default-REAL contract the explicit `REAL` and `REAL(8)` forms are type-equivalent;
- under four-byte default REAL they are not.

This strengthens the `BUILD_CONTRACT_DEPENDENCY` classification. It does not establish that every implicit call site is semantically safe, because no explicit interface enforces the result kind.

The audit also currently observes zero possible calls to `fexp` and `fexph`. That is insufficient to label them redundant or dead code; call detection is conservative and source-selection provenance remains unresolved.

Qualification tests for the auditor: 3/3 synthetic unit tests PASS, and regeneration of the persisted source audit from the frozen source archive matches exactly.
...
Intel storage-semantics review narrows the interpretation. Intel default `/Qauto-scalar` behaviour puts non-SAVEd scalar INTEGER, REAL, COMPLEX and LOGICAL locals on the stack, while the relevant `Outbal_write` values are CHARACTER scalars. Their persistence can therefore be compatible with Intel default storage semantics without requiring a historical `/Qsave` setting.

Classification:

`BUILD_CONTRACT_DEPENDENCY`

The GNU `-fno-automatic` switch is a broad diagnostic emulation aid and is not evidence that the historical Intel build used `/Qsave`. The first native reference candidate must test Intel default storage semantics; `/Qsave` is a sensitivity variant only if needed.
...
| external function result crosscheck | IMPLEMENTED_FOR_19_RECONSTRUCTABLE_FUNCTIONS | BUILD_CONTRACT_DEPENDENCY_AT_DBLE_TRUNC |
...
| inconsistent subroutine call sites | PARTIALLY_PROBED | NOT_ASSESSED |
...
The paired GNU probes and the external-function audit demonstrate that default kind selection is part of the behavioural build contract. Precision modernization therefore cannot be treated as a mechanical declaration cleanup.
...
- local-storage duration semantics in the current evidence-derived diagnostic contract.

`tools/build_gnu_diagnostic.py` records these dependencies as an explicit, source-hash-bound diagnostic recipe. `docs/prep01/HISTORICAL_INTEL_BUILD_CONTRACT.md` records the leading Intel 12.1 reference-build hypothesis. Neither claims historical equivalence yet.

## Next interface-audit step

The external-function crosscheck is now complete at its declared static-analysis level. The next useful interface work is semantic audit of the largest subroutine interfaces and call sites.
...
- whether it is scientifically meaningful, I/O/configuration-only, diagnostics-only, scratch, or genuinely redundant.

The first candidates are `Input1` and `Outbal_calc`, but mass-balance semantics should be mapped before shortening `Outbal_calc`, because many of its arguments may encode explicit conservation terms rather than accidental plumbing.
