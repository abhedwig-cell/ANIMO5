...
gfortran -std=f2008 -Wall -Wextra -Werror -ffp-contract=off -fno-fast-math -J "$build" -I "$build" \
  ...
  prototype/kt18/mod_animo_provider_backed_application.f90 \
  tests/kt18/test_kt18_provider_backed_application.f90 \
  -o "$build/test_kt18"

"$build/test_kt18"

# Provider and application regressions remain required because KT18 adds a
# read-only selection seam to KT11 and consumes KT15A.
bash tests/kt11/run_kt11_tests.sh
bash tests/kt15/run_kt15_tests.sh
