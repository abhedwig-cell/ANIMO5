...
gfortran -std=f2008 -Wall -Wextra -Werror -ffp-contract=off -fno-fast-math -J "$build" -I "$build" \
  ...
  tests/kt17/test_kt17_checkpoint_manifest_integrity.f90 \
  -o "$build/test_kt17"

"$build/test_kt17"

# Regression: the introspection-only KT15 change must not alter KT15/KT16 behavior.
bash tests/kt15/run_kt15_tests.sh
bash tests/kt16/run_kt16_tests.sh
