program test_kt14_boundary_frame_composition
  ...
  call test_valid_exact_frame_commits()
  call test_stale_frame_rejects_before_science()
  call test_mutated_frame_provenance_rejects()
  call test_dry_deposition_is_not_wet_advective_forcing()
  print *, 'PASS_KT14_BOUNDARY_FRAME_TCD042_COMPOSITION'
...
contains
  ...
  subroutine make_frame(t0,t1,frame)
    ...
    call bind_static_boundary_interval(boundary,'KT14-STATIC-BOUNDARY',2000,cursor, &
      t0,t1,frame,next,ok,reason)
    call assert_true(ok,'bind BOUNDQ02 frame')
  ...
  subroutine test_valid_exact_frame_commits()
    ...
    call make_frame(t0,t1,frame)
    call initialize_tcd042_store(store,'KT14-VALID',t0,1.0_real64,ok)
    call assert_true(ok,'initialize valid store')

    call execute_boundary_frame_tcd042_interval(store,packet, &
      'ANIMO_PG_86400_NOLEAPSECONDS_V1',0_int64,t0,t1,'KT14-VALID-EXEC', &
      start,frame,1,1.0_real64,trace,success,reason)

    call assert_true(success,'valid frame composition commits')
    call assert_true(trace%boundary_frame_validated,'frame validated before science')
  ...
  subroutine test_stale_frame_rejects_before_science()
    ...
    call make_start(start)
    call make_frame(t0,t1,frame)
    call initialize_tcd042_store(store,'KT14-STALE',t0,2.0_real64,ok)
    call assert_true(ok,'initialize stale-frame store')

    call execute_boundary_frame_tcd042_interval(store,packet, &
      'ANIMO_PG_86400_NOLEAPSECONDS_V1',0_int64,t0,t2,'KT14-STALE-EXEC', &
      start,frame,1,1.0_real64,trace,success,reason)

    call assert_true(.not.success,'stale interval frame rejected')
  ...
  end subroutine test_stale_frame_rejects_before_science

  subroutine test_mutated_frame_provenance_rejects()
    type(TimeCoordinate)::t0,t1
    type(hydrology_step_t)::packet
    type(hydroexec01_start_context_t)::start
    type(static_boundary_interval_frame_t)::frame
    ...
    call initialize_tcd042_store(store,'KT14-MUTATED',t0,3.0_real64,ok)
    call assert_true(ok,'initialize mutated-frame store')

    call execute_boundary_frame_tcd042_interval(store,packet, &
      'ANIMO_PG_86400_NOLEAPSECONDS_V1',0_int64,t0,t1,'KT14-MUTATED-EXEC', &
      start,frame,1,1.0_real64,trace,success,reason)

    call assert_true(.not.success,'mutated frame provenance rejected')
    call assert_true(.not.trace%boundary_frame_validated,'mutated frame rejected before science')
    call assert_store_unchanged(store,730119_int64,3.0_real64)
  end subroutine test_mutated_frame_provenance_rejects

  subroutine test_dry_deposition_is_not_wet_advective_forcing()
    type(TimeCoordinate)::t0,t1
    type(hydrology_step_t)::packet1,packet2
    type(hydroexec01_start_context_t)::start
    type(static_boundary_interval_frame_t)::frame1,frame2
    ...
    call make_start(start)
    call make_frame(t0,t1,frame1)
    frame2=frame1
    frame2%dry_deposition_nh=99.0_real64
    frame2%dry_deposition_ni=88.0_real64
    call initialize_tcd042_store(store1,'KT14-DRY-A',t0,1.0_real64,ok)
    call assert_true(ok,'initialize dry A')
    call initialize_tcd042_store(store2,'KT14-DRY-B',t0,1.0_real64,ok)
    call assert_true(ok,'initialize dry B')

    call execute_boundary_frame_tcd042_interval(store1,packet1, &
      'ANIMO_PG_86400_NOLEAPSECONDS_V1',0_int64,t0,t1,'KT14-DRY-A-EXEC', &
      start,frame1,1,1.0_real64,trace1,success1,reason)
    call execute_boundary_frame_tcd042_interval(store2,packet2, &
      'ANIMO_PG_86400_NOLEAPSECONDS_V1',0_int64,t0,t1,'KT14-DRY-B-EXEC', &
      start,frame2,1,1.0_real64,trace2,success2,reason)
